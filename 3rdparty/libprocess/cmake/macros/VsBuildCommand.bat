...
for /F "skip=2 tokens=1,2*" %%A in ('REG QUERY %VS2015_KEY% /v %VS2015_INSTALLDIR_VALUE% 2^>nul') do (
  set VS2015_DIR=%%C..\..
)
...
FINDSTR ^
    ...
    %SOLUTION_FILE:/=\%
if %errorlevel% neq 0 (
  :: Upgrade solution file if its version not match current %SOLUTION_VER%
